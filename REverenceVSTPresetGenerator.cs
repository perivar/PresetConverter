...
class Script
{
    ...
    // some impulses have similiar names but lives in different folders, then prefix vst name with parent-folder-name
    ...
    static public void CommandLine(string[] args)
    {
        ...
        // use windows form dialog boxes instead
        //_wavDir = PromptForPath(_wavDir, "Select Impulse Responses Directory ...");
        //_presetDir = PromptForPath(_presetDir, "Select directory where to save the .vstpreset files ...");
        //_imageFilePath = PromptForFile(Path.GetDirectoryName(_imageFilePath), "Choose image file to use on all exported .vstpreset files ...", "Image Files(*.BMP;*.JPG;*.GIF)|*.BMP;*.JPG;*.GIF|All files (*.*)|*.*");
        ...
        try
        {
            ...
            foreach (string wavFilePath in wavFilePaths)
            {
                ...
                if (_altiverbImageMode)
                {
                    ...
                    FileInfo[] imageFiles = new DirectoryInfo(wavFilePath).Parent.GetFiles("*.jpg");
                    ...
                    if (null != imageFiles && imageFiles.Length > 0)
                    {
                        ...
                        {
                            //if (fi.FullName.Contains("stats.jpg")) continue;
                ...
                {
                    audioChannels = wavReader.NumberOfChannels;
    ...
    public bool Process()
    {
        ...
        try
        {
            ...
            {    // check for chunks inside RIFF data area. 
                ...
                {    // if NOT the fmt or LIST chunks just skip over the data.
...
public class PresetFile
{
    ...
    public bool EndChunk(Entry e)
    {
        ...
        //Console.Out.WriteLine("EntryCount: {0}", entryCount);
        return true;
    }
