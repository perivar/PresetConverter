...
namespace CSCore.Codecs.WAV
{
    ...
    public class RIFFFileReader
    {
        ...
        public RIFFFileReader(string fileName)
        {
            if (fileName == null)
            ...
            else if (firstChunkId == "IMPS")
            {
                 ...
                 * 79: vib rate
                 */
                string fileName2 = Encoding.ASCII.GetString(binaryFile.ReadBytes(12));
                binaryFile.ReadByte(); // skip bytes
                byte globalVol = binaryFile.ReadByte();
                byte flags = binaryFile.ReadByte();
                int channels = 0;
                int bitsPerSample = 0;
                ...
                byte defaultVol = binaryFile.ReadByte();
                string sampleName = Encoding.ASCII.GetString(binaryFile.ReadBytes(26)).TrimEnd('\0');
                byte convert = binaryFile.ReadByte();
                byte defaultPan = binaryFile.ReadByte();
                int lengthInSamps = binaryFile.ReadInt32();
                int loopStart = binaryFile.ReadInt32();
                int loopEnd = binaryFile.ReadInt32();
                int sampleRate = binaryFile.ReadInt32();
                int sustainLoopStart = binaryFile.ReadInt32();
                int sustainLoopEnd = binaryFile.ReadInt32();
                int dataLocation = binaryFile.ReadInt32();
                byte vibSpeed = binaryFile.ReadByte();
                byte vibDepth = binaryFile.ReadByte();
                byte vibWave = binaryFile.ReadByte();
                byte vibRate = binaryFile.ReadByte();
        ...
        /// <summary>
        /// Reads a sequence of bytes from the <see cref="RIFFFileReader" /> and advances the position within the stream by the
        ...
        /// <summary>
        /// Gets or sets the position of the <see cref="RIFFFileReader" /> in bytes.
        /// </summary>
        ...
        /// <summary>
        /// Gets the length of the <see cref="RIFFFileReader" /> in bytes.
        /// </summary>
        public long Length
        {
        ...
        /// <summary>
        /// Gets a value indicating whether the <see cref="RIFFFileReader"/> supports seeking.
        /// </summary>
        public bool CanSeek
        {
            get { return true; }
        ...
        private List<WaveFileChunk> ReadChunks(BinaryFile binaryFile)
        {
            var chunks = new List<WaveFileChunk>(2);
            do
            {
                var tmp = WaveFileChunk.FromBinaryFile(binaryFile);
                chunks.Add(tmp);
